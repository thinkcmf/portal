...
class ProfileController extends UserBaseController
{
    ...
    // 用户头像Blob上传
    // author:zeyax6@gmail.com
    // create:2019/10/19 03:19
    public function avatarUpload()
    {
        // 判断用户本月是否修改过头像
        $userId = cmf_get_current_user_id();
        $res = Db::name("user")->field('avatar')->where("id", $userId)->find();
        if(empty($res)) $this->error("数据不存在。");

        // 获取上传的图像
        $file = $this->request->file('file');

        // 判断是否为空
        if(empty($file)) $this->error("数据不存在。");
        ...
        // 验证结果集
        if (!$result) $this->error($file->getError());
        ...
        $result     = $avatarImg->save($avatarPath);

        // 判断结果
        if(!$result) $this->error("头像保存失败！");
